...
copy_shared_pages(envid_t child)
{
    ...
    int err;
    void *addr;
    // loop through all PTE in current process (like fork) and copy any page mappings that have PTE_SHARE set into child process
    for (uint64_t va = (uint64_t)0; va < UTOP;) {
        if (uvpde[VPDPE(va)] & PTE_P) {
            if (uvpd[VPD(va)] & PTE_P) {
                uint64_t stop = va + (1 << PDXSHIFT);
                for (; va < stop; va += PGSIZE) {
                    if ((uvpt[VPN(va)] & (PTE_P | PTE_SHARE)) == (PTE_P | PTE_SHARE)) {
                        err = sys_page_map(0, (void *)va, child, (void *)va, uvpt[VPN(va)] & PTE_SYSCALL);
                        if (err < 0)
                            return err;
                    }
                }
