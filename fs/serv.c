...
serve_read(envid_t envid, union Fsipc *ipc)
{
    ...
    // copy what other functions here do...
    struct OpenFile *o;
    int err;
    uint32_t size;

    // look up file ID
    err = openfile_lookup(envid, req->req_fileid, &o);
    if (err < 0)
        return err;
    
    // read file
    // be careful if req->req_n > PGSIZE
    size = (req->req_n > PGSIZE) ? PGSIZE : req->req_n;
    err = file_read(o->o_file, ret->ret_buf, size, o->o_fd->fd_offset);
    if (err < 0)
        return err;
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
    ...
    // LAB 5: Your code here.
    // jchung: basically follows serve_read
    struct OpenFile *o;
    int err;
    uint32_t size;
    ...
    // be careful if req->req_n > PGSIZE
    size = (req->req_n > PGSIZE) ? PGSIZE : req->req_n;
    err = file_write(o->o_file, req->req_buf, size, o->o_fd->fd_offset);
    if (err < 0)
        return err;
