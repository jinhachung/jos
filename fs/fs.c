...
alloc_block(void)
{
    ...
    for (size_t i = 0; i < super->s_nblocks; ++i) {
        if (block_is_free(i)) {
            // do opposite of free_block
            bitmap[i / 32] &= ~(1 << (i % 32));
            // immediately flush changed bitmap block to disk
    ...
    if (filebno < NDIRECT) {
        ...
        *ppdiskbno = &(f->f_direct[filebno]);
        ...
        if (f->f_indirect == 0) {
            ...
            blockno = alloc_block();
            if (blockno < 0)
                // but no space on disk for an indirect block
                return -E_NO_DISK;
...
file_get_block(struct File *f, uint32_t filebno, char **blk)
{
    // LAB 5: Your code here.
    uint32_t *p;
    int blockno = file_block_walk(f, filebno, &p, 1);
    if (blockno < 0)
    ...
    // set *blk to the address in memory where f[filebnbo] will be mapped
    *blk = (char *)diskaddr(*p);
    return 0;
}
